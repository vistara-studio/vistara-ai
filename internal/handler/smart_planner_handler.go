...
// SmartPlannerHandler handles smart travel planning HTTP requests
type SmartPlannerHandler struct {
...
// NewSmartPlannerHandler creates a new instance of SmartPlannerHandler
func NewSmartPlannerHandler(smartPlannerService *service.SmartPlannerService, integrationService *service.IntegrationService, logger *logger.Logger, cfg *config.Config) *SmartPlannerHandler {
	return &SmartPlannerHandler{
...
func (h *SmartPlannerHandler) GenerateSmartPlan(c *fiber.Ctx) error {
	// Parse request body
...
func (h *SmartPlannerHandler) validateSmartPlanRequest(req *dto.SmartPlanRequest) error {
	// Calculate duration from dates
	duration := int(req.EndDate.Sub(req.StartDate).Hours()/24) + 1
...
func (h *SmartPlannerHandler) cleanAIResponse(response string) string {
